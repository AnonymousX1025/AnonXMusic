...
repo(){
	...
	[[ ! "AnonXMusic" == $(basename -s .git `git config --get remote.origin.url`) ]] &&
	git clone https://github.com/AnonymousR1025/AnonXMusic &&  cd AnonXMusic
}
...
OWNER_ID = $ownid""" > .env
clear
pprint "\n\n\nYour Vars have been saved Successfully!, Thanks for using AnonXMusic Installer, now you can proceed by starting the bot with bash start!"
pprint "\n\n\nWant more vars?"
