...
class TgCall(PyTgCalls):
    ...
    async def play_media(self, chat_id: int, media, message=None):
        ...
        try:
            ...
            if message:
                try:
                    await message.edit_text(f"🚀 **Playing Next:**\n\n📌 **Title:** {media.title}", reply_markup=buttons.controls(chat_id))
                except: pass
    ...
    async def play_next(self, chat_id: int):
        ...
        if not media:
            if await db.is_autoplay_mode(chat_id):
                logger.info(f"Autoplay trigger: {chat_id}")
        ...
        msg = await app.send_message(chat_id, f"🔄 **Next Track:**\n🎵 **{media.title}**")
    ...
    async def boot(self) -> None:
        ...
        logger.info("Direct Streamer & Autoplay Ready!")
