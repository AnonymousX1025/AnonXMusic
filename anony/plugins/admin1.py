...
@app.on_message(filters.command(["رفع MY"], "") & filters.channel)
...
@app.on_message(filters.command(["رفع Dev"], "") & filters.group)
def promote_g_admin(client, message):
    ...
    for tom in tooom:
    	if tom.user.id == tom_id and (tom.status == enums.ChatMemberStatus.OWNER or tom.status == enums.ChatMemberStatus.ADMINISTRATOR):
    		client.promote_chat_member(chat_id, user_id, ToM)
    		message.reply(f"◍ تم رفع {user_id} Dev بنجاح \n\n √")
...
@app.on_message(filters.command(["رفع M"], "") & filters.group)
def promote_g_admin(client, message):
    if message.reply_to_message and message.reply_to_message.from_user:
        target = message.reply_to_message.from_user.id
        user_id = str(target)
    ...
    for tom in tooom:
    	if tom.user.id == tom_id and (tom.status == enums.ChatMemberStatus.OWNER or tom.status == enums.ChatMemberStatus.ADMINISTRATOR):
    		client.promote_chat_member(chat_id, user_id, ToM)
    		message.reply(f"◍ تم رفع {user_id} M بنجاح \n\n √")
