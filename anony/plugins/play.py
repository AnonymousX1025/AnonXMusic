...
@app.on_message(
    (filters.regex(r"^(تشغيل|شغل|vplay|vplayforce)$") |
     filters.regex(r"^(تشغيل|شغل|vplay|vplayforce) .+"))
    & filters.group
    & ~app.bl_users
)
@lang.language()
@checkUB
async def play_hndlr(_, m: types.Message, force: bool = False, m3u8: bool = False, video: bool = False, url: str = None) -> None:
