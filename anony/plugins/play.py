...
@app.on_message(filters.text & filters.group & ~app.bl_users) @lang.language() @checkUB async def play_hndlr(_, m: types.Message) -> None: text = (m.text or m.caption or "").strip() if not text: return parts = text.split() cmd = parts[0].lstrip("/").lower() triggers = ["تشغيل", "شغل", "vplay", "vplayforce"] if cmd not in triggers: return
